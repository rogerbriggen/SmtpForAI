...
The server might later also provide a MCP interface, allowing it to be used with Minecraft servers and plugins. This will allow players to send emails from within the game, providing a fun and interactive way to use the SMTP server.

We want this tool easy and secure, so try to not use any third party libraries beside from the Microsoft ones.